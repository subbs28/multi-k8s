...
kubectl apply -f k8s
kubectl set image deployments/server-deployment server=subbs28/multi-server:$SHA
kubectl set image deployments/client-deployment client=subbs28/multi-client:$SHA
kubectl set image deployments/worker-deployment worker=subbs28/multi-worker:$SHA